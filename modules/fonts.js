...
const getFontFaceSetFonts = async (win, list) => {
	const controlledErrorMessage = 'FontFaceSet.check blocked or not supported'
	try {
		const gibberish = '&WY2tR*^ftCiMX9LD5m%iZSWCVSg'
		if (win.document.fonts.check(`12px '${gibberish}'`)) {
			throw new Error(controlledErrorMessage)
		}
		await win.document.fonts.ready
		// Actual usage of this requires clean scopes (iframes or workers)
		// For example, show fonts loaded on the page and there should be none
		//console.log([...document.fonts.values()].map(fontFace => fontFace.family)) 
		win.document.fonts.clear() // clear loaded or added fonts
		const fonts = list.filter(font => win.document.fonts.check(`12px '${font}'`))
		return fonts
	} catch (error) {
...
const getFontFaceLoadFonts = async (win, list) => {
	try {
...
export const getFonts = async imports => {
	...
	try {
		...
		const [
			...
			getFontFaceLoadFonts(win, [...getFontList(), ...getExtendedFontList()]),
			getFontFaceSetFonts(win, getAllFonts())
		])
...
export const fontsHTML = ({ fp, note, modal, count, hashSlice, hashMini }) => {
	...
	const platformHash = hashMini(platform)
	const loadListLen = [...getFontList(), ...getExtendedFontList()].length
	return `
	<div class="col-six">
		<strong>Fonts</strong><span class="hash">${hashSlice($hash)}</span>
		<div class="help" title="CSSStyleDeclaration.setProperty()\nwidth\nheight\ntransform-origin\nperspective-origin\nblock-size\ninline-size">platform (${platformFonts ? count(platformFonts) : '0'}/${''+getSystemFonts().length}): ${
		...
		<div class="help" title="FontFace.load()">load (${fontFaceLoadFonts ? count(fontFaceLoadFonts) : '0'}/${''+loadListLen}): ${
			fontFaceLoadFonts.length ? modal(
				'creep-fonts-load', fontFaceLoadFonts.map(font => `<span style="font-family:'${font}'">${font}</span>`).join('<br>'),
				hashMini(fontFaceLoadFonts)
			) : note.unsupported
		}</div>
		<div class="help" title="FontFaceSet.check()">check (${fontFaceSetFonts ? count(fontFaceSetFonts) : '0'}/${''+getAllFonts().length}): ${
